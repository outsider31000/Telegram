local redemrp = Config.redm -- SET TO FALSE IF YOU USE VORP
						...
						for k, v in pairs(players) do
							TriggerEvent("vorp:getCharacter", v, function(user)
								...
								if receiver == firstname .. " " .. lastname then 
									TriggerClientEvent("vorp:Tip", v, Config.sent, 3000)
								end
							end)
						end
					end)
					TriggerClientEvent("vorp:Tip", _source, Config.notification, 3000)
				else 
					print(recipient)
					TriggerClientEvent("vorp:Tip", _source, Config.unable, 3000)
				end
			end)
		end)
	end
